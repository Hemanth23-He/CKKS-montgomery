...
class Polynomial:
    ...
    def scalar_multiply(self, scalar, coeff_modulus=None):
        ...
        if coeff_modulus:
            ...
            new_coeffs = []
            scalar_mont = reducer.to_montgomery(int(scalar))
            for c in self.coeffs:
                c_mont = reducer.to_montgomery(int(c))
                prod_mont = reducer.montgomery_mul(scalar_mont, c_mont)
                prod = reducer.from_montgomery(prod_mont)
                new_coeffs.append(prod % coeff_modulus)
        else:
