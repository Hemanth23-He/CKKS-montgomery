...
def sample_uniform(min_val, max_val, num_samples):
    ...
    """
    if num_samples == 1:
        #return random.SystemRandom().randrange(min_val, max_val)"""A module to sample randomly from various distributions."""
import random
...
def sample_hamming_weight_vector(length, hamming_weight):
    ...
    Samples uniformly from the set [-1, 0, 1] such that the resulting vector has exactly h nonzero values.
    Args:
